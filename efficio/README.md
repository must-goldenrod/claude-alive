...
'낭비'는 **2차원**이다(MTMM, docs 13.5). 두 축군을 분리해 함께 본다:
...
> **단일 '주축' 없음.** MTMM(3방법)에서 두 객관 방법(rework·Bash)이 서로 수렴(+0.43)하며 주관 라벨과는 음으로 갈림(−0.52, −0.36) → **체감 낭비(W2)와 행동 낭비(WC·Bash)는 다르고 심지어 반대.** 어느 게 '진짜 낭비'인지는 가치판단(사용자 몫). 기본 표시 축은 W2(검증 우위 아님). 근거: docs 13.2~13.5.

## 사용법

```bash
# 의존성: python3 stdlib만 (numpy 불필요 — math/statistics로 순화). sqlite3도 stdlib.
