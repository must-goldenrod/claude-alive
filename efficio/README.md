...
| 축 | 의미 | 검증 상태 (두 기준 갈림) |
|---|---|---|
| **W2 컨텍스트 재무효화** | 워밍업 제외 cache_creation 토큰의 크기보정 잔차 | 주관 H1 상관(ρ≈0.57)·**객관 rework와 무관** |
| W3 재탐색 | 동일 파일 반복 read | 미검증(둘 다 약) |
| WC 편집 반복 | 동일 파일 반복 edit | **객관 rework와 약상관(ρ≈0.19)**·주관 약 |

> **단일 '주축' 없음.** 자기평정(H1)은 W2를, 객관 rework는 WC를 지지 — 서로 다른 차원이라 함께 본다(검증 근거: docs 13.2~13.4). 기본 표시 축은 신호 밀도가 높은 W2(검증 우위 아님).
