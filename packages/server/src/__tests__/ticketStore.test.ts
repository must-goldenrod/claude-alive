...
describe('createTicketStore', () => {
  ...
  it('snapshots the run preset into concrete model/effort at creation', async () => {
    ...
    expect(t).toMatchObject({ preset: 'deep', requestedModel: 'opus', effort: 'max' });

    const onDisk = JSON.parse(await readFile(filePath, 'utf-8'));
    expect(onDisk[0]).toMatchObject({ preset: 'deep', requestedModel: 'opus', effort: 'max' });
  });
  ...
  it('keeps the snapshot on reload so a retry reuses the original profile', async () => {
    ...
    const b = makeStore();
    await b.load();
    expect(b.list()[0]).toMatchObject({ preset: 'fast', requestedModel: 'sonnet', effort: 'low' });
  });
