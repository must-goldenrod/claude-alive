...
export function EventStream({ events, agents }: EventStreamProps) {
  ...
  const agentNameMap = useMemo(() => {
    ...
    for (const a of agents) {
      map.set(a.sessionId, a.displayName || a.projectName || t('agents.generalAgent'));
    }
    return map;
  }, [agents]);
    ...
    >
      ...
      <div className="overflow-y-auto flex-1 p-3 space-y-0.5">
        ...
        {events.map((entry) => {
          ...
          const agentName = agentNameMap.get(entry.sessionId) ?? t('agents.generalAgent');
          ...
          return (
            <div
              key={entry.id}
              className="flex items-center gap-3 px-3.5 py-2.5 rounded-lg text-xs hover:bg-[var(--hover)] transition-colors"
            >
